...
| Kingbase | `KES V9R1C10` | 单机 | 物理备份、逻辑恢复与迁移 | `TestKingbaseBackupRestoreAndMigrationUseControlledArguments`、`TestKingbaseRefusesUnsafeLifecyclePITRAndUpgrade` | 待提供受控备份目录、独立恢复实例与数据校验集；PITR 需正式 repository/archive 配置，升级需经官方前置核验 |
| Kingbase | `KES V9R1C10` | 单机 | 监控、teardown 拒绝与多节点边界 | `TestKingbaseMonitorCollectsFixedSingleNodeSnapshot`、`TestKingbaseTeardownRequiresConfirmationBackupAndVerifiedUninstall`、`TestKingbaseRejectsReplicationFailoverAndRebuild` | 监控需可访问单机实例；公开 V9R1C10 官方入口未提供可核验卸载程序与参数，teardown 保持零命令拒绝，等待厂商书面流程 |
...
| GBase 8a | `MPP Cluster V9` | 单机共置专属 Agent | 部署、配置、L0 备份、受控恢复、LOAD DATA 迁移、监控、teardown 边界 | `TestGBase8aDeploysOnlyWithApprovedManifestAndCommands`、`TestGBase8aBackupRestoreAndMigrationUseControlledCommands`、`TestGBase8aMonitorsWithFixedStatusProcessAndCapacityCommands`、`TestGBase8aTeardownRequiresConfirmationFinalBackupAndVerifiedUninstall`、`TestGBase8aRejectsDistributedLifecycleOperations` | 官方 V9 公开下载中心和产品页核验 `gcadmin status`、GNode/GCluster/GCware 及容量监控对象；公开资料缺少可核验卸载程序和参数，teardown 在确认和最终备份输入通过后保持零命令拒绝；待提供厂商书面卸载流程、已校验介质、许可证、备份目录和目标主机 |

运行 `make test-xinchuang-lifecycle` 可执行已完成 flavor 的 Agent 生命周期夹具、内核 Agent 路由契约、后端 capability 门禁和前端 capability 镜像测试。GBase 8a、openGauss 与 GBase 8s 的已实现操作仍通过专属 flavor Agent 路由执行，通用 MySQL API 与前端操作保持禁用直到完成后端路由接入。真实环境验收必须记录实际安装包 SHA-256、运行日期、目标架构、任务标识和每个阶段的 Agent 结果。
