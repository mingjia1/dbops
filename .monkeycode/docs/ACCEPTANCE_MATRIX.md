...
| GBase 8a | `10.1` | 单机专属 Agent | 部署、配置、L0 备份、受控恢复、LOAD DATA 迁移、TLS 与升级边界 | `TestGBase8aDeploysOnlyWithApprovedManifestAndCommands`、`TestGBase8aConfigureUsesStrictGlobalAllowlist`、`TestGBase8aBackupRestoreAndMigrationUseControlledCommands`、`TestGBase8aLifecycleFailureReturnsNormalModeAndRejectsUnsafeInputs`、`TestGBase8aTLSConfigContentTargetsGBasedSection` | 待提供已核验安装介质、许可证、受控备份目录和目标主机；TLS 等待厂商确认重启命令，升级和回滚等待厂商核验复杂拓扑与版本条件 |
